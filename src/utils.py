...
class BaseDataset:
    ...
    @classmethod
    def flatten_ground_truth(cls, ground_truth):
        """Flatten answers into a flat list of strings.

        Supports: str, list[str], list[list[str]], tuple/set variants, and None.
        """
        if ground_truth is None:
            return []
        if isinstance(ground_truth, str):
            return [ground_truth]
        if not isinstance(ground_truth, (list, tuple, set)):
    ...
    def exact_match_score(
        ...
        ground_truths = {
            str(gt).strip()
            for gt in cls.flatten_ground_truth(ground_truth)
...
def evaluate(pred, ground_truth, with_cot=False):
    ...
    ground_truths = [
        str(gt).strip()
        for gt in BaseDataset.flatten_ground_truth(ground_truth)
        if gt is not None and str(gt).strip()
    ]

    # Some datasets/splits (e.g., BioASQ phase-A) do not contain gold answers.
    # Return non-evaluable markers so inference can continue without crashing.
    if not ground_truths:
        return {
            "eval_predict": pred,
            "em": "nan",
