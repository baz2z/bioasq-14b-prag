...
def main(args):
    ...
    # Search for warmup weights: type-specific > dataset-level > generic > random init
    init_adapter_path = None
    warmup_base = os.path.join(
        ROOT_DIR,
        "warmup",
        "lora_base_weight",
        args.model_name,
    ...
    # 1. Type-specific warmup (e.g. warmup/.../bioasq_summary/)
    if args.data_type:
        candidate = os.path.join(warmup_base, f"{args.dataset}_{args.data_type}")
        if os.path.exists(os.path.join(candidate, "adapter_model.safetensors")):
            init_adapter_path = candidate
            print(f"Using type-specific warmup weights: {init_adapter_path}")
    # 2. Dataset-level warmup (e.g. warmup/.../bioasq/)
    if init_adapter_path is None:
        candidate = os.path.join(warmup_base, args.dataset)
        if os.path.exists(os.path.join(candidate, "adapter_model.safetensors")):
            init_adapter_path = candidate
            print(f"Using dataset-level warmup weights: {init_adapter_path}")
    # 3. Generic warmup (e.g. warmup/.../direct/)
    if init_adapter_path is None:
        candidate = os.path.join(warmup_base, "direct")
        if os.path.exists(os.path.join(candidate, "adapter_model.safetensors")):
            init_adapter_path = candidate
            print(f"Using generic warmup weights: {init_adapter_path}")
    # 4. Fallback: random init in offline/
    if init_adapter_path is None:
        ...
        if not os.path.exists(
            ...
            print("No warmup weights found, creating random LoRA base weight...")
            ...
            print(f"Using existing random base weight: {init_adapter_path}")
